...
## 内容创作
终于来到相对轻松的环节了~因为 Docusaurus 的初衷就是让内容创作者专注于内容，只需编写 Markdown 文件，所以只要你在 `docs` 或者 `blog` 文件夹里新建 md 文件，编写内容后，再根据上篇 [博客](/blog/docusausus-github) 完成构建和部署，就可以成功发布上线啦！
